...
// DUMMY_DETAILS의 키 타입을 정의합니다.
type PlaceId = 'place1' | 'place2' | 'place3' | 'place4';

// [핵심] 여러 장소의 상세 데이터를 포함하는 객체입니다. Record 유틸리티 타입을 사용하여 키와 값의 타입을 명시합니다.
const DUMMY_DETAILS: Record<PlaceId, PlaceDetail> = {
  place1: {
    ...
    stats: { likes: 14, comments: 5, bookmarks: 7 },
    comments: [
        {
          id: 'comment1',
...
const CommentItem = ({ comment, isReply = false }: { comment: Comment, isReply?: boolean }) => {
    return (
        <View style={{ marginLeft: isReply ? 30 : 0, marginTop: 16 }}>
            ...
            {comment.replies?.map((reply: Comment) => ( // [오류 수정] reply 타입 지정
...
export default function PlaceDetailScreen({ route }: any) {
  ...
  // [오류 수정] route.params에서 오는 id의 타입을 명시적으로 지정합니다.
  const { id } = route.params as { id: PlaceId };

  // 전달받은 id를 사용하여 DUMMY_DETAILS에서 해당 장소 정보를 찾습니다.
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
        ...
        <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
            ...
            {/* [오류 수정] map 함수의 파라미터 타입을 string으로 지정합니다. */}
            ...
            {/* [오류 수정] map 함수의 파라미터 타입을 Comment로 지정합니다. */}
